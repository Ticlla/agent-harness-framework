...
3. **§6 delegates resolve (install-dir-authoritative)** — for every delegate named in §6 / `references/skill-delegation.md`:
   - **Resolve the path §6 records**, exactly. §6 must record the **install-dir path** (e.g. `~/.claude/skills/<name>`; `~/.cursor/skills/` for Cursor targets) — that is where the runtime loads delegates by name. Confirm that path has a **readable** `SKILL.md` — a dir with no readable `SKILL.md` is a fail.
   - A §6 entry whose recorded path is a **sibling-repo path** (`../<repo>/skills/...`) instead of an install-dir path = **fail**: the runtime does not scan sibling repos. Fix: install or symlink the delegate into the install dir (recorded in §6) and re-validate. Flag it as an environment dependency either way.
